  ...
  module Formatters
    ...
    module ErrorMapper
      ...
      def self.registration_failed(node_name, exception, config)
        ...
        headline = "#{Chef::Dist::EXEC} encountered an error attempting to create the client \"#{node_name}\""
      ...
      def self.node_load_failed(node_name, exception, config)
        ...
        headline = "#{Chef::Dist::EXEC} encountered an error attempting to load the node data for \"#{node_name}\""
        description = ErrorDescription.new(headline)
        error_inspector.add_explanation(description)
        description
      end
