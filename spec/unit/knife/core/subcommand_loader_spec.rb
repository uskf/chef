...
describe Chef::Knife::SubcommandLoader do
  ...
  describe "#for_config" do
    context "when ~/.chef/plugin_manifest.json exists" do
      before do
        allow(File).to receive(:exist?).with(File.join(home, Chef::Dist::USER_CONF_DIR, "plugin_manifest.json")).and_return(true)
      end

      it "creates a HashedCommandLoader with the manifest has _autogenerated_command_paths" do
        allow(File).to receive(:read).with(File.join(home, Chef::Dist::USER_CONF_DIR, "plugin_manifest.json")).and_return("{ \"_autogenerated_command_paths\": {}}")
    ...
    context "when ~/.chef/plugin_manifest.json does not exist" do
      before do
        allow(File).to receive(:exist?).with(File.join(home, Chef::Dist::USER_CONF_DIR, "plugin_manifest.json")).and_return(false)
      end
