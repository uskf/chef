      ...
      describe "ChefConfig::Config[:internal_locale]" do
        ...
        shared_examples_for "a suitable locale" do
          it "returns an English UTF-8 locale" do
            expect(ChefConfig.logger).to_not receive(:warn).with(/Please install an English UTF-8 locale for #{Chef::Dist::EXEC} to use/)
        ...
        context "when the result does not include 'en_*.UTF-8'" do
          ...
          it "should fall back to C locale" do
            expect(ChefConfig.logger).to receive(:warn).with("Please install an English UTF-8 locale for #{Chef::Dist::EXEC} to use, falling back to C locale and disabling UTF-8 support.")
